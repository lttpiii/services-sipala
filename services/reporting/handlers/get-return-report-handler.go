...
func (h *ReportingHandler) GetReturnReportHandler(c *gin.Context) {
	log.Printf("[get-return-report] hit service get return report with query %v\n", c.Request.URL.Query())
	...
	var parsedQuery types.DTOGetReturnReport
	if err := c.ShouldBindQuery(&parsedQuery); err != nil {
		log.Printf("[get-return-report] failed to bind query: %v", err)
	...
	log.Printf("[get-return-report] get return report success")
